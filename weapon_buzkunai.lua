...
if (CLIENT) then
	killicon.Add( "weapon_buzkunai", "icons/weapon_ninjaskunai_killicon", color_white )
	...
	if (CLIENT) then
		...
		if !game.SinglePlayer() then
			hook.Add( "PlayerButtonDown", "MAS.Keybind.NinjasKunai" .. ply:EntIndex(), function( ply, btn )
				...
				if ( ply:IsValid() and ply:Alive() and Wep:IsValid() and Wep:GetClass() == "weapon_buzkunai" ) then
		...
		hook.Add( "PlayerBindPress", "MAS.Keybind.NinjasKunai" .. ply:EntIndex(), function( ply, bind, down )
			local Wep = ply:GetActiveWeapon()
			if ( ply and ply:IsValid() and ply:Alive() and MASconvar_preventbinds:GetBool() and Wep:IsValid() and ply:GetActiveWeapon():GetClass() == "weapon_buzkunai" ) then
...
function SWEP:DealDamage( ability )
	...
	elseif ability == ASWEP_Ability_ThrowKunai then // end if Auto Attack
		...
		DoElementalEffect( { Element = EML_ICE, Target = tr.Entity, Duration = 10, Attacker = self.Owner } )
...
function SWEP:NinjaDash( ply, self, Wep )
	...
	local Wep = ply:GetActiveWeapon()
		...
		if Wep and Wep:GetClass() == "weapon_buzkunai" then
			...
			hook.Add( "Move", "MAS.Ability.NinjaDash.Cast" .. ply:EntIndex(), function( ent, data )
				...
				if ply:IsValid() and Wep:IsValid() and self:GetA2_Active() then
					// Weapon Check
					if ( Wep:GetClass() != "weapon_buzkunai" ) then
...
function SWEP:Concealment( ply, self, Wep )
	-- variable check
	local ply	=	ply or self.Owner
	local Wep	=	Wep or ply:GetWeapon( "weapon_buzkunai" )
				...
				if ent == ply and ent != atk then
					...
					timer.Create( "MAS.Ability.Substitution.Death" .. ply:EntIndex(), 1, maxIt, function()
						...
						if ply:IsValid() and ply:Alive() then -- if Ply
							local SWEP = ply:GetWeapon( "weapon_buzkunai" )
...
function SWEP:Think()
	...
	if CLIENT and game.SinglePlayer() then
		...
		if !vgui.CursorVisible() and !vgui.GetKeyboardFocus() and self.KB then
			...
			local Wep = ply:GetActiveWeapon()
			if !( ply:IsValid() and ply:Alive() and Wep:IsValid() and Wep:GetClass() == "weapon_buzkunai" ) then return end
...
if (SERVER) then
	...
	net.Receive( "Maranzo_AbilitySWEP_ninjaskunai_Cast", function( len, ply )
		...
		if ( IsValid( ply ) and ply:Alive() and ply:GetActiveWeapon():GetClass() == "weapon_buzkunai" ) then
