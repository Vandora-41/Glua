...
PrecacheParticleSystem("silver_app")
...
function Spell:SetEnabled(val)
	if val then
		...
		self.UseWhiteSmoke = tobool(self.Owner:GetInfo("silver_app"))
		...
		timer.Simple(FrameTime() * 3, function()
			if IsValid(e) then HpwRewrite.MakeEffect(self.UseWhiteSmoke and "silver_app" or "silver_app", nil, nil, e, PATTACH_ABSORIGIN_FOLLOW) end
		end)
	else
...
HpwRewrite:AddSpell("Silver Apparition", Spell)