...
PrecacheParticleSystem("vandorap")
PrecacheParticleSystem("vandora_impact")
...
function Spell:SetEnabled(val)
	if val then
		...
		self.UseWhiteSmoke = tobool(self.Owner:GetInfo("vandorap"))
		...
		timer.Simple(FrameTime() * 3, function()
			if IsValid(e) then HpwRewrite.MakeEffect(self.UseWhiteSmoke and "vandorap" or "vandorap", nil, nil, e, PATTACH_ABSORIGIN_FOLLOW) end
		end)
	else
...
function Spell:Think()
	...
	if self.Owner:IsOnGround() or self.Owner:WaterLevel() > 2 then 
		...
		HpwRewrite.MakeEffect("vandora_impact", self.Owner:GetPos())
...
HpwRewrite:AddSpell("Vandora Apparition", Spell)